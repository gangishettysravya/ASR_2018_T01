...
def _preprocess_data(args):
    ...
    timit_df.to_hdf("./features/"+path+"/timit.hdf", "timit")
    #timit_df.to_hdf("./features/"+path+"/test.hdf", "test")
    
    #Phone-wise dumping into folders
    ...
    for phone in phones:
        indices = np.where(mfcc_labels==phone)
        features_ph = []
        for i in indices:
            features_ph.append(mfcc_features[i])
        ph_df = pd.DataFrame()
        ph_df["features"] = features_ph
        if(phone != ''):
            ph_df.to_hdf("./features/"+path+"/phone_wise/"+phone+".hdf",phone)
        else:
            ph_df.to_hdf("./features/"+path+"/phone_wise/"+"space.hdf","space")
